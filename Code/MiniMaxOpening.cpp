 ...
 * MiniMax Opening Functionality for Morris Variant Game
 */
...
output_class minimax_opening(string board_position, int height, string flag)
{
    ...
    for(auto boards : board_vector)
        ...
        {
            //cout<<boards<<endl;
            child_node = minimax_opening(boards, height - 1, "min");
        ...
        {
            //cout<<boards<<endl;
            child_node = minimax_opening(boards, height - 1, "max");
            if(child_node.estimate < curr_node.estimate)
            {
                curr_node.estimate = child_node.estimate;
                curr_node.board_state = boards;
...
int main(int argc, char *argv[])
{
   ...
   int height = stoi(argv[3]);
    ...
    output_class opc_obj = minimax_opening(input_string, height, "max");
