 ...
 * MiniMax Opening Black functionality for Morris Variant Game
 */
...
{
    return (count(board_position.begin(), board_position.end(), 'B') - count(board_position.begin(), board_position.end(), 'W'));
}

output_class minimax(string board_position, int height, string flag)
{
    ...
    for(auto boards : board_vector)
        if(flag == "max")
        {
            child_node = minimax(boards, height - 1, "min");
        ...
        {
            child_node = minimax(boards, height - 1, "max");
            if(child_node.estimate < curr_node.estimate)
            {
                curr_node.estimate = child_node.estimate;
                curr_node.board_state = boards;
...
int main(int argc, char *argv[])
{
   ...
   int height = stoi(argv[3]);
    ...
    output_class opc_obj = minimax(input_string, height, "max");
